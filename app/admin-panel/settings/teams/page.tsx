...
export default async function TeamsPage() {
  ...
  const [{ data: teams, error }, { data: memberRows }, { data: accountRows }] =
    await Promise.all([
      admin.from("teams").select("id, name, plan").order("name", { ascending: true }),
  ...
  for (const r of accountRows ?? []) {
    accountCounts.set(r.team_id, (accountCounts.get(r.team_id) ?? 0) + 1);
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      <div>
        ...
        <h2 style={{ fontSize: 18, fontWeight: 600, margin: 0 }}>Teams</h2>
      ...
      <section style={cardStyle}>
        ...
        <p style={{ margin: "4px 0 16px", fontSize: 13, color: "#71717a" }}>
          Name it now — you can add accounts and invite people next.
        </p>
        <CreateTeamForm />
      </section>
      ...
      <section style={cardStyle}>
        ...
        {rows.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {rows.map((t) => (
              <Link key={t.id} href={`/app/settings/teams/${t.id}`} style={teamRowStyle}>
                ...
                <span style={{ fontSize: 13, color: "#a1a1aa" }}>Manage &rarr;</span>
              </Link>
            ))}
          </div>
        )}
